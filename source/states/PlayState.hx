...
import utility.StateIntercepter;
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		ModSourceCodeLoader.callOnAll("onCreate", []);
		...
		// for lua
		instance = this;
		ModSourceCodeLoader.setOnAll("game", instance);
		...
		if(ClientPrefs.data.allowSmoothHealth || ClientPrefs.data.allowVslice){
		...
		ModSourceCodeLoader.callOnAll("onCreatePost", []);
	...
	public function addTextToDebug(text:String, color:FlxColor) {
		ModSourceCodeLoader.callOnAll("addTextToDebug", [text, color]);
	...
	public function reloadHealthBarColors() {
		ModSourceCodeLoader.callOnAll("reloadHealthBarColors", []);
		healthBar.setColors(FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]),
			FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]));
	}

	public function addCharacterToList(newCharacter:String, type:Int) {
		ModSourceCodeLoader.callOnAll("addCharacterToList", [newCharacter, type]);
		switch(type) {
			case 0:
	...
	{
		ModSourceCodeLoader.callOnAll("startVideo", [name, forMidSong, canSkip, loop, playOnLoad]);
		#if VIDEOS_ALLOWED
	...
	{
		ModSourceCodeLoader.callOnAll("startDialogue", [dialogueFile, song]);
	...
	public function startCountdown()
	{
		...
		if(ret != LuaUtils.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				ModSourceCodeLoader.callOnAll("onCountdownTick", [tick, swagCounter]);
	...
	public function startNextDialogue() {
		...
		ModSourceCodeLoader.callOnAll("onNextDialogue", [dialogueCount]);
	}

	public function skipDialogue() {
		callOnScripts('onSkipDialogue', [dialogueCount]);
		ModSourceCodeLoader.callOnAll("onSkipDialogue", [dialogueCount]);
	}
	...
	function startSong():Void
	{
		...
		ModSourceCodeLoader.callOnAll("onSongStart", []);
	}
	...
	function makeEvent(event:Array<Dynamic>, i:Int)
	{
		...
		ModSourceCodeLoader.callOnAll("onEventPushed", [subEvent.event, subEvent.value1 != null ? subEvent.value1 : '', subEvent.value2 != null ? subEvent.value2 : '', subEvent.strumTime]);
	}
	...
	override function closeSubState()
	{
		...
		if (paused)
		{
			...
			ModSourceCodeLoader.callOnAll("onResume", []);
	...
	override public function update(elapsed:Float)
	{
		...
		ModSourceCodeLoader.callOnAll("onUpdate", [elapsed]);

		if(ClientPrefs.data.allowSmoothHealth || ClientPrefs.data.allowVslice) smoothHealth = FlxMath.lerp(smoothHealth, health, .2 / (ClientPrefs.data.framerate / 60));
		...
		if (unspawnNotes[0] != null)
		{
			...
			while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time)
			{
				...
				ModSourceCodeLoader.callOnAll("onSpawnNote", [dunceNote]);
		...
		ModSourceCodeLoader.callOnAll("onUpdatePost", [elapsed]);
	}
	...
	function openChartEditor()
	{
		...
		MusicBeatState.switchState(StateIntercepter.checkOverride("ChartingState", new ChartingState()));
	}
	...
	function openCharacterEditor()
	{
		...
		MusicBeatState.switchState(StateIntercepter.checkOverride("CharacterEditorState", new CharacterEditorState(SONG.player2)));
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		{
			...
			if(ret != LuaUtils.Function_Stop)
			{
				ModSourceCodeLoader.callOnAll("onGameOver", [null, true]);
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		ModSourceCodeLoader.callOnAll("onEvent", [eventName, value1, value2, strumTime]);
	}
	...
	public function endSong()
	{
		...
		{
			ModSourceCodeLoader.callOnAll("onEndSong", []);
			#if !switch
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					canResync = false;
					MusicBeatState.switchState(StateIntercepter.checkOverride("StoryMenuState", new StoryMenuState()));
			...
			else
			{
				...
				canResync = false;
				MusicBeatState.switchState(StateIntercepter.checkOverride("FreeplayState", new FreeplayState()));
				FlxG.sound.playMusic(Paths.music('freakyMenu'));
				changedDifficulty = false;
			}
			transitioning = true;
	...
	private function keyPressed(key:Int)
	{
		...
		else
		{
			if (ClientPrefs.data.ghostTapping){
				...
				ModSourceCodeLoader.callOnAll("onGhostTap", [key]);
			} else {
				noteMissPress(key);
			}
		}
		...
		ModSourceCodeLoader.callOnAll("onKeyPress", [key]);
	}
	...
	private function keyReleased(key:Int)
	{
		...
		ModSourceCodeLoader.callOnAll("onKeyRelease", [key]);
	}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if(result != LuaUtils.Function_Stop && result != LuaUtils.Function_StopHScript && result != LuaUtils.Function_StopAll) callOnHScript('noteMiss', [daNote]); ModSourceCodeLoader.callOnAll("noteMiss", [daNote]);
	}
	...
	function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
	{
		...
		ModSourceCodeLoader.callOnAll("noteMissPress", [direction]);
	...
	{
		// score and data
		ModSourceCodeLoader.callOnAll("noteMissCommon", [direction, note]);
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		else if(!note.noAnimation)
		{
			...
			if(char != null)
			{
				...
				if(canPlay) {
					if(ClientPrefs.data.allowVslice && !note.isSustainNote){
						char.playAnim(animToPlay, true); // Prevents from playing more animations if the sustain is keep alive
					} else if(!ClientPrefs.data.allowVslice) char.playAnim(animToPlay, true);
				}
				char.holdTimer = 0;
			}
		}
		...
		if(result != LuaUtils.Function_Stop && result != LuaUtils.Function_StopHScript && result != LuaUtils.Function_StopAll) callOnHScript('opponentNoteHit', [note]); ModSourceCodeLoader.callOnAll("opponentNoteHit", [note]);
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(!note.hitCausesMiss) //Common notes
		{
			...
			if (ClientPrefs.data.allowVslice && note.isSustainNote && !cpuControlled)
			{
				songScore++;
				updateScoreText();
			}
		...
		if(result != LuaUtils.Function_Stop && result != LuaUtils.Function_StopHScript && result != LuaUtils.Function_StopAll) callOnHScript('goodNoteHit', [note]); ModSourceCodeLoader.callOnAll("goodNoteHit", [note]);
		if(!note.isSustainNote) invalidateNote(note);
	}

	public function invalidateNote(note:Note):Void {
		ModSourceCodeLoader.callOnAll("invalidateNote", [note]);
		note.kill();
	...
	public function spawnNoteSplashOnNote(note:Note) {
		ModSourceCodeLoader.callOnAll("spawnNoteSplashOnNote", [note]);
		if(note != null) {
	...
	public function spawnNoteSplash(x:Float = 0, y:Float = 0, ?data:Int = 0, ?note:Note, ?strum:StrumNote) {
		ModSourceCodeLoader.callOnAll("spawnNoteSplash", [x, y, data, note, strum]);
	...
	override function destroy() {
		ModSourceCodeLoader.callOnAll("onDestroy", []);
	...
	override function stepHit()
	{
		...
		ModSourceCodeLoader.callOnAll("onStepHit", []);
	}
	...
	override function beatHit()
	{
		...
		ModSourceCodeLoader.callOnAll("onBeatHit", []);
	}

	public function characterBopper(beat:Int):Void
	{
		ModSourceCodeLoader.callOnAll("characterBopper", [beat]);
	...
	{
		ModSourceCodeLoader.callOnAll("playerDance", []);
	...
	override function sectionHit()
	{
		...
		ModSourceCodeLoader.callOnAll("onSectionHit", []);
	}

	#if LUA_ALLOWED
	public function startLuasNamed(luaFile:String)
	{
		ModSourceCodeLoader.callOnAll("startLuasNamed", [luaFile]);
		#if MODS_ALLOWED
	...
	{
		ModSourceCodeLoader.callOnAll("startHScriptsNamed", [scriptFile]);
		#if MODS_ALLOWED
	...
	public function initHScript(file:String)
	{
		...
		ModSourceCodeLoader.callOnAll("initHScript", [file]);
		try
		{
	...
	public function callOnScripts(funcToCall:String, args:Array<Dynamic> = null, ignoreStops = false, exclusions:Array<String> = null, excludeValues:Array<Dynamic> = null):Dynamic {
		...
		ModSourceCodeLoader.callOnAll("callOnScripts", [funcToCall, args, ignoreStops, exclusions, excludeValues]);
		if(args == null) args = [];
	...
	public function callOnLuas(funcToCall:String, args:Array<Dynamic> = null, ignoreStops = false, exclusions:Array<String> = null, excludeValues:Array<Dynamic> = null):Dynamic {
		var returnVal:Dynamic = LuaUtils.Function_Continue;
		ModSourceCodeLoader.callOnAll("callOnLuas", [funcToCall, args, ignoreStops, exclusions, excludeValues]);
		#if LUA_ALLOWED
		if(args == null) args = [];
		if(exclusions == null) exclusions = [];
		if(excludeValues == null) excludeValues = [LuaUtils.Function_Continue];
	...
	public function callOnHScript(funcToCall:String, args:Array<Dynamic> = null, ?ignoreStops:Bool = false, exclusions:Array<String> = null, excludeValues:Array<Dynamic> = null):Dynamic {
		var returnVal:Dynamic = LuaUtils.Function_Continue;
		ModSourceCodeLoader.callOnAll("callOnHScript", [funcToCall, args, ignoreStops, exclusions, excludeValues]);
	...
	public function setOnScripts(variable:String, arg:Dynamic, exclusions:Array<String> = null) {
		ModSourceCodeLoader.callOnAll("setOnScripts", [variable, arg, exclusions]);
		if(exclusions == null) exclusions = [];
	...
	public function setOnLuas(variable:String, arg:Dynamic, exclusions:Array<String> = null) {
		#if LUA_ALLOWED
		ModSourceCodeLoader.callOnAll("setOnLuas", [variable, arg, exclusions]);
		if(exclusions == null) exclusions = [];
		for (script in luaArray) {
			if(exclusions.contains(script.scriptName))
				continue;
	...
	public function setOnHScript(variable:String, arg:Dynamic, exclusions:Array<String> = null) {
		#if HSCRIPT_ALLOWED
		ModSourceCodeLoader.callOnAll("setOnHScript", [variable, arg, exclusions]);
		if(exclusions == null) exclusions = [];
		for (script in hscriptArray) {
	...
	public function RecalculateRating(badHit:Bool = false, scoreBop:Bool = true) {
		ModSourceCodeLoader.callOnAll("RecalculateRating", [badHit, scoreBop]);
	...
	{
		ModSourceCodeLoader.callOnAll("createRuntimeShader", [shaderName]);
		#if (!flash && sys)
	...
	{
		ModSourceCodeLoader.callOnAll("initLuaShader", [name, glslVersion]);
